...
class LinkedInPublisher:
    ...
    def _create_post(self, text: str, title: Optional[str] = None, 
                    visibility: str = 'PUBLIC', 
                    author_urn: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        if title:
            data['specificContent']['com.linkedin.ugc.ShareContent']['shareCommentary']['text'] = f"{title}\n\n{text}"
    ...
    def publish_to_organization(self, organization_id: str, 
    ...
    def _create_organization_post(self, organization_id: str, text: str, 
    ...
    def get_followers_count(self, organization_id: str = None) -> int:
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                return data.get('followerCount', 0) if organization_id else data.get('numFollowers', 0)
            else:
                raise APIError(
